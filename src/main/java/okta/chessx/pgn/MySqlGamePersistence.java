...
import static okta.chessx.engine.utils.Utils.PrintLn;

public class MySqlGamePersistence implements PGNPersistence {
    private Connection dbConnection;

    private MySqlGamePersistence INSTANCE = new MySqlGamePersistence();
    private static final String JDBC_DRIVER = "com.mysql.cj.jdbc.Drive";
    private static final String DB_URL = "jdbc:mysql://localhost/chessgame";
    ...
    private MySqlGamePersistence() {
        ...
        createGameTable();
        createIndex("outcome", "OutcomIndex");
    ...
    private void createIndex(final String columnName, final String indexName) {
        try {
            final String sqlString =
                    "SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_CATALOG = 'def' AND " + "TABLE_SCHEMA = " +
                            "DATABASE() AND TABLE_NAME = \"game\" AND INDEX_NAME = \"" + indexName + "\"";
            ...
            PrintLn("CREATE INDEX " + indexName + " on Game)" + columnName + ");\n");
            e.printStackTrace();
        }
    }
